...
type ChannelTwilioSMS struct {
	...
	PhoneNumber            string    `gorm:"column:phone_number;not null;uniqueIndex:idx_channel_twilio_sms_phone_number" json:"phone_number"` // Renomeado
...
func (ChannelTwilioSMS) CreateIndexes(db *gorm.DB) {
	...
	db.Migrator().CreateIndex(&ChannelTwilioSMS{}, "idx_channel_twilio_sms_account_sid_phone_number") // Renomeado
	// Criar índice único para o campo messaging_service_sid
	db.Migrator().CreateIndex(&ChannelTwilioSMS{}, "idx_channel_twilio_sms_messaging_service_sid") // Renomeado
	// Criar índice único para o campo phone_number
	db.Migrator().CreateIndex(&ChannelTwilioSMS{}, "idx_channel_twilio_sms_phone_number"); // Renomeado
}